...
path_xcl = forms.pick_file(files_filter=filterXcl, title="SELECT EXCEL FILE")

if not path_xcl:
	script.exit()

# Prompt user to specify families
path_rfas = forms.pick_file(files_filter=filterRfa, multi_file=True, title="SELECT FAMILIES")

if not path_rfas:
	script.exit()

# Import Excel data
from Almond_xclUtils import *

xcl = xclUtils([],path_xcl)
dat = xcl.xclUtils_import("Standard", 5, 11)

targets_params, target_bipgs, fam_inst, fam_formula = [],[],[],[]

for row in dat[0][1:]:
	targets_params.append(row[0])
	target_bipgs.append(row[2])
	fam_inst.append(row[3] == "INSTANCE")
	fam_formula.append(row[4])
...
with forms.ProgressBar(step=1, title="Updating families", cancellable=True) as pb:
	...
	for filePath in path_rfas:
		...
		if pb.cancelled:
			...
			if famDoc != None:
				pars = famDoc_addSharedParams(famDoc, fam_defs, fam_bipgs, fam_inst, fam_formula)
